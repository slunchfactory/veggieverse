...
interface SpiritFinderProps {
  headerOffset?: number;
}

const SpiritFinder: React.FC<SpiritFinderProps> = ({ headerOffset = 96 }) => {
  ...
  useEffect(() => {
    ...
    const initialItems: FloatingItem[] = PRODUCE_ITEMS.map((produce, index) => {
      ...
      const baseSize = 90;
      const scale = (1.8 + Math.random() * 1.2) * sizeMultiplier;
      const baseSpeed = 12 + Math.random() * 18;
  ...
  useEffect(() => {
    ...
    const animate = (time: number) => {
      ...
      const height = window.innerHeight - headerOffset + 150;
      const wrapMargin = 220;
      const minSpeed = 10;
      const maxSpeed = 35;
